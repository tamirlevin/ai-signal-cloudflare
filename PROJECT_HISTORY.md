...
## 26 September 2026 — Jev question study and reader-taste review (source only)
...
- Source changes include migration `0009_jev_human_reviews.sql`; no remote migration was applied and no deployment was made. The current staging migration state and post-change live behavior remain unverified.
...
- Owner verdicts on Jev/gate disagreements shipped with snapshot freezing and automatic agreement math. At the time, the trial proposed a 14-day promotion rule; the later review design above supersedes that proposal for the current experiment. Migration `0007_jev_verdicts` was applied to staging D1 (via execute; the migrations endpoint returned 7403, so a future `migrations apply` will harmlessly no-op on `IF NOT EXISTS`).
