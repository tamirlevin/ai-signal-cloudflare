...
- Owner verdicts on Jev/gate disagreements ship with snapshot freezing, a staging `/admin` panel, and automatic rule math. Fixed promotion rule: over 14 days the owner sides with Jev on at least 70% of disagreements with fewer than one wanted story in Jev's confident rejects. Migration `0007_jev_verdicts` is applied to staging D1 (via execute; the migrations endpoint returned 7403, so a future `migrations apply` will harmlessly no-op on `IF NOT EXISTS`).
