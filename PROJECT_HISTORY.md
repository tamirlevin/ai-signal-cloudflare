...
## 26 September 2026 — Jev question study and reader-taste review (staging)

- Jev and the reranker remain advisory shadow tools. The owner-facing review batch samples up to 12 Jev-scored candidates, up to three from each pipeline-selected/JeV-publish, pipeline-selected/JeV-reject, pipeline-excluded/JeV-publish, and pipeline-excluded/JeV-reject group, then fills remaining places from the rest of the scored pool. Candidates with no usable evidence and Jev rows with no answer are omitted.
- The owner labels every item publish, reject, or unsure, then ranks only publish choices. A saved row is keyed by shadow-run ID and story URL, so the same story in another run can receive another label. The candidate context, reranker/Jev outputs, exact question payload, question-set version, profile version, and source-pack version are snapshotted alongside it. Edits update the current label/rank without rewriting that model snapshot.
- Question usefulness and reader taste are tracked as separate learning questions. Jev remains shadow-only; the existing binary disagreement verdicts and their old promotion metrics are a legacy calibration record, not an activation criterion for this workflow. Bump `JEV_QUESTION_SET_VERSION` whenever question meaning or intended use changes.
- Commit `d69eea8f5e691364a0520ee34bbe18e3e9145f1c` was pushed to `feature/mts-lane` and deployed only to staging. Deployment `22033476-f8d8-4d06-a372-307971dba015` serves version `f5f12b81-d2ae-4acb-b74f-055108137bb2` with tag `git-d69eea8-staging`; the prior staging deployment for rollback is `1c138352-ab1d-4479-ae69-d27f219bcc70`, version `1c3c7ecf-7af2-40c9-84b6-d2516e19e8a2`.
- Staging D1 already contained the schema effects of `0007_jev_verdicts.sql` and `0008_jev_reader_wants.sql`, but its Wrangler ledger stopped at `0006_model_attempt_audit.sql`. Read-only schema checks confirmed the Jev verdict table and `jev_reader_wants` column were present. The two existing migration names were added to the staging ledger; this reconciled metadata only. Wrangler then applied `0009_jev_human_reviews.sql` to D1 `ai-signal-staging` (`d1d32bf8-9edf-463a-b151-8ea54abc2e4d`) and captured its normal migration backup. No review labels or application rows were written.
- Release checks passed generated Worker types, TypeScript, all 138 tests in 17 files, staging dry-run packaging, and diff validation. After deployment, health, status, latest-edition, and shadow endpoints returned HTTP 200; `/admin` returned 200 and served the new review UI assets. The review GET and POST endpoints returned 401 without an owner token, as required. A read-only schema check found the 14-column review table and zero saved labels.
- One documented staging-only `/__shadow` QA run produced shadow report `85aedaeb-67aa-4c40-b3ba-227c2e7d8076` at `2026-09-26T05:23:00.785Z`, with 34 Jev scores and question-set version `reader-want-v1`. Its status is degraded because AI Secret returned no recognized source-linked items; the other six source lanes were healthy. Running the deterministic selector against this live report produced 12 review candidates, exactly three from each selected/excluded × Jev-publish/reject group. The route wrote a shadow report only; the latest edition was not regenerated or republished. The protected batch and save endpoints were not exercised with an owner token, and no review labels were saved.
- Production remained untouched: GitHub `main` is still `28fdd29f9235bf65bf2714c2fe25fba5a4b80e39`, and the latest production deployment record remains version `01f557a2-313f-4ad7-9b29-4a3ce928e241` from commit `003b4ece6587017776d5cdd573e6aac0f0bca4d8`.

## 24 September 2026 — staging trial: coverage gate, pack v7, Jev judge, verdicts (unmerged)

- All work below is on the unmerged branch `feature/mts-lane`; production (`main`, source pack v5) is untouched. Staging (`testsignal.tamirlevin.dev`, D1 `ai-signal-staging`) carries the trial.
- A staging 9/24 edition published 11 of 12 selected candidates. The missing card ("Better GPT-6 Prompt Caching") was traced to silent collector product-version merging (`gpt:6` claimed first by "GPT-6 Sol and Luna"), not the model or normalization. Merge decisions are now recorded everywhere they happen (`duplicate-url`, `duplicate-title`, `duplicate-text`, `product-version` with surviving targets, plus invalid rejections), logged per run, and enforced: any materialized candidate with no card and no recorded decision throws a coverage gap that fails the run loudly. Fail-loud applies on both envs; no warn-and-publish path exists.
- Source pack v7 adds AI Brief (daily runs JSON API) as an equal discovery lane under unchanged evidence, freshness, and profile-fit gates. First staging shadow run: 15 fetched → 10 accepted → 4 in-window → 4 qualified → 4 selected, all sources healthy. Social-only items are skipped; X links remain discovery-ineligible as evidence pending a dedicated trial.
- TypeSafe Jev (`typesafe/jev`) is reachable two ways: Cloudflare Workers AI needs separate third-party billing (blocked with `Insufficient AI Gateway credits`), while the direct API works with a `TYPESAFE_API_KEY` secret. First live call on a real story returned `reader_wants 0.91`, `materially_new 0.56`, `substantive 0.68` in 616 ms at ~509/59 tokens. The Jev shadow judge now scores every evaluated story per shadow run (interest choice over reader interests plus watching topics with a none option, pre-today novelty, substantive scoring, all with confidence) alongside the reranker; first run scored 28/28 rows with zero novelty zeros.
- Owner verdicts on Jev/gate disagreements shipped with snapshot freezing and automatic agreement math. At the time, the trial proposed a 14-day promotion rule; the later review design above supersedes that proposal for the current experiment. Migration `0007_jev_verdicts` and the `0008` reader-wants column were applied directly to staging before their names were recorded in Wrangler's migration ledger; that ledger mismatch was reconciled during the 26 September staging rollout.
- Staging cadence moved from 2-hourly to 8-hourly (`15 */8 * * *` UTC): 2-hour intervals showed no meaningful pool change between runs. Side effect: the 15-row shadow retention now covers ~5 days instead of ~30 hours, which nearly fits the calibration week in D1.
- Prompt-leak class fixes verified by test, pending live proof at the next successful generation: the model is told never to repeat word limits or category tags, and synthesis source labels are overwritten with collector publisher names instead of model text (`...`, `(source)`).

## 20 September 2026 — AlphaSignal repair and source-funnel observability

- Read-only D1 evidence showed AlphaSignal failing its initial sitemap request on 11 consecutive scheduled runs from 9–19 September after working through 8 September. The configured historical sitemap was not recoverable through a retry: a final direct check returned 6,283 URLs / 2,166,994 bytes, above the Worker's 2 MB limit, after 23.24 seconds to first byte, beyond the 15-second abort.
- Source pack v5 replaces that endpoint with AlphaSignal's Google News sitemap. A repeated pre-change check returned 10 recent entries / 4,974 bytes after 10.71 seconds. The parser now prefers genuine `news:publication_date` and `news:title` fields while retaining `lastmod` and URL-derived titles for legacy fixtures. The 15-second limit remains unchanged and there is no retry; Cloudflare-egress latency remains a production unknown until a scheduled v5 run.
- Source errors now identify fetch/read/parse, sitemap, issue, yield, or per-host enrichment stages. Source health is independent of yield: a transport or parse failure remains failed, recognized-but-empty output is quiet and healthy or degraded according to layout certainty, and one quiet/failed lane remains non-blocking.
- Each source report gains a compact candidate funnel: accepted candidates remain the existing top-level count; the additive funnel records distinct in-window clusters, same-source merges, qualified and selected clusters, plus outside-window, unusable-evidence, weak-profile-fit, and ranked-out exclusions. Cross-source merges preserve each source's participation rather than being treated as discarded candidates.
- AInews now enforces its existing 8 MB limit while streaming bytes instead of buffering the entire body before a character-count check. Its issue-level timestamp policy is unchanged; the observed duplicate/misdated RSS publication dates are documented but no slug-based selection system was added.
- Pre-release verification passed generated Worker types, TypeScript, all 106 tests in 12 files, dry-run packaging, diff validation, and a read-only live-pool rehearsal. The rehearsal fetched 10 AlphaSignal entries, enriched eight candidates without errors, raised the unchanged-profile pool from six to ten eligible candidates inside 72 hours, and produced three AlphaSignal-led cards among nine rendered cards.
- Source commit `003b4ece6587017776d5cdd573e6aac0f0bca4d8` was pushed to GitHub `main` before strict deployment. The rollback point is deployment `ad180077-2b2b-432a-ad98-fbd0cef06454`, version 44 `445c5fa4-3622-4e02-ac8d-9a71aa320f9b`. Remote D1 reported no pending migrations, and none was applied.
- Deployment `b3143bf8-fc5b-4aaa-b094-d0a19a88109a` serves 100% through version 45 `01f557a2-313f-4ad7-9b29-4a3ce928e241`, created `2026-09-20T00:25:10.394067Z`, with verified tag `git-003b4ec` and full-SHA message. The custom domain, `15 22 * * *` UTC schedule, bindings, and secret binding are unchanged.
- Health, status, latest-edition, profile, and shadow endpoints returned HTTP 200 after deployment. Edition `2d446698-2614-4e59-9c30-275a678c681d`, profile v7, and shadow `dfa86aa5-db2c-40f1-923a-de1966065b0a` remain unchanged; no generation, D1 write, or republish was triggered. Production execution of source pack v5, including Cloudflare-egress AlphaSignal timing and the new funnels, remains pending the 21 September 08:15 Australia/Melbourne scheduled run.
...
- Source commit `91a4db9596539051cfc470e9cdfb68cccaabf6bd` was pushed before strict deployment. Deployment `ad180077-2b2b-432a-ad98-fbd0cef06454` serves 100% through version 44 `445c5fa4-3622-4e02-ac8d-9a71aa320f9b`, created `2026-09-08T23:39:50.340031Z`, with verified tag `git-91a4db9` and full SHA metadata. Domain, schedule, bindings and secrets are unchanged.
- The guarded three-statement profile batch succeeded through Wrangler. Public profile v7 exactly matches v6 except its version and coding/new-systems/research weights (3/3/2); the old v6 row is retained. Health, status, latest-edition, profile and shadow returned HTTP 200. Edition `cffde492-b794-48b7-b25d-b226b5185cbd` is still the six-story 9 September issue; no generation or republish was invoked. On 10 September, inspect the new edition, v4 source report including AI Secret yield, profile v7 snapshot, and editorial QA logs; deployment health alone does not prove those scheduled behaviors.
...
- Source commit `b62db62d0977fe41f75b2fa9ba04f3459fb5c403` was pushed before strict deployment. Deployment `da2d9f78-197e-4b47-8138-4c2ca53ca9eb` serves 100% through version 43 `b05461db-6411-48f0-b162-7f5f8c54f189`, created `2026-09-08T22:49:05.741058Z`, with verified tag `git-b62db62` and full SHA metadata. Health, status, latest-edition, profile, and shadow returned HTTP 200; the custom domain and `15 22 * * *` UTC schedule are unchanged.
- Edition `cffde492-b794-48b7-b25d-b226b5185cbd` remains the six-story 9 September issue. A read-only D1 query wrote zero rows and confirmed the morning cron (`2026-09-08T22:15:36.353Z`–`22:15:56.241Z`) is still the latest run. No migration, secret change, or republish occurred. First live QA execution and model effectiveness remain pending the 10 September 08:15 Australia/Melbourne run; inspect its `ai-signal editorial QA` log and published synthesis rather than inferring success from the cron heartbeat alone.
...
- The owner-authorized ordinary refresh was invoked at `2026-08-31T03:19:11.164Z` and authenticated successfully. It failed safely after 1,946 ms with `VALIDATION_FAILED` because no qualified story remained inside the 48-hour window; Workers AI was not invoked and no edition or manual-republish claim was written. At `03:20:39Z`, the cutoff was `2026-08-29T03:20:39Z`: the earlier qualifying AlphaSignal/OpenAI candidate had aged to roughly 49.6 hours, AInews and TLDR were stale, Cloudflare had no current candidate, and AlphaSignal's only fresher item scored 2 and did not meet the existing profile-fit gate. The 26 August last-good edition and healthy scheduled heartbeat were preserved. The production failure path is verified, but a successful first `daily-pool` edition remains unresolved until a qualifying source candidate appears.
