...
export function editorialMessages(issue: RssIssue, profile: Profile, repair?: string): Array<{ role: "system" | "user"; content: string }> {
  ...
  return [
    ...
    {
      role: "user",
      content: `Create only the editorial framing and cross-story synthesis for AI Signal Profile v${profile.version}. The collector—not you—will create Hot Topics and individual signal cards directly from the candidate inventory. Return exactly these top-level fields: schemaVersion, issue, presentation, synthesis. Do not return hotTopics or signals.\n\nUse plain text, not HTML or Markdown, in every title, lead, kicker and body. Write compactly: synthesis lead and big picture at most 60 words each; section titles at most 12 words; section kickers at most 18 words; section bodies at most 90 words. Create 2-3 synthesis sections. Use no more than 6 sources for the overall synthesis and no more than 3 sources for any section. Connect distinct stories into useful themes rather than repeating one announcement. Favor practical agents, Codex and agent craft, new systems, integrations and AI business implications. Keep raw model scores, local setup, video, routine policy/cyber/infrastructure and pre-training details in the background unless exceptionally consequential.\n\nCRITICAL LINK CONTRACT: Every sources[].url must be copied byte-for-byte from the Allowed direct links below. Never invent, shorten, redirect, canonicalize, or modify URLs. The issue URL (${issue.url}) may appear only in issue.url, never in sources. Preserve uncertainty with phrases such as 'AInews reports' or 'the announcement claims'.\n\nActive profile:\n${profileJson}\n\nIssue metadata: publicationDate=${issue.publicationDate}; coverage should be inferred only from the issue; url=${issue.url}\n\nAllowed direct links:\n${allowedLinks}\n\nRanked AInews candidate inventory:\n${issue.body}\n${repairInstruction}`
    }
  ];
}

export function generationInput(issue: RssIssue, profile: Profile, repair?: string, useStructuredOutput = true): ChatCompletionsMessagesInput {
  ...
  return { ...input, reasoning_effort: "low", chat_template_kwargs: { enable_thinking: false }, response_format: { type: "json_schema", json_schema: { name: "ai_signal_edition", strict: true, schema: EDITION_SCHEMA } } };
}
